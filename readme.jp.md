...
すべてのことには理由がある。  
このプロジェクトは、実際の業務で何度も直面した痛みから生まれました。特に **フォームや大規模リスト** におけるレンダリング粒度の制御です。

私は Vue (Vue2) 出身です。当時、リアクティブの仕組みはあまり理解していませんでした。

- Vue2 の data はリアクティブですが、`this` が後から注入されるのは直感的ではありませんでした。
- Vue3 では `reactive/ref` が登場し、「なぜデータがリアクティブになるのか」を強く理解したくなりました。
- そこで最小実装を作りました：Proxy で `track` と `trigger`、`effect` 内で依存収集。データが変わると再レンダリング。
- この時はじめて「**データ自体は環境に依存せず、effect に置かれることで意味を持つ**」と理解しました。

これが後の **templify-form** の原型になりました。
